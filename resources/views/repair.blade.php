                            ...
                            @foreach($data as $index => $datas)
                                <tr>
                                    ...
                                    <td style="text-align: center">
                                        <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#detail-{{$datas->id}}" href="{{route('repair.onEdit', ['id' => $datas->id])}}"><span class="glyphicon glyphicon-wrench"></span></button>
                                    </td>
                                    <td style="text-align: center">
                                        {{--<form action="{{route('repair.delete', ['id' => $datas->id])}}">--}}
                                        <button type="submit" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#delete-{{$datas->id}}" href="{{route('repair.onEdit', ['id' => $datas->id])}}"><span class="glyphicon glyphicon-trash"></span></button>
                                        {{--</form>--}}
                                    </td>
                                </tr>
                            @endforeach
...
<div class="modal fade" id="addData" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST" action="{{route('repair.save')}}">
            {{ csrf_field() }}
        ...
        <div class="modal fade" id="detail-{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form method="POST" action="{{route('repair.edit', ['id' => $item->id])}}">
                        {{ csrf_field() }}
                        <div class="modal-body">
        ...
        <div class="modal fade" id="delete-{{$itemOne->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form method="POST" action="{{route('repair.delete', ['id' => $itemOne->id])}}">
                        {{ csrf_field() }}
                        <br>
